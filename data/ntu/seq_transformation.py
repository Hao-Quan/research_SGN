...
save_path = '/media/hao/My Passport/projects/SGN'
...
def align_frames(skes_joints, frames_cnt):
    ...
    for idx, ske_joints in enumerate(skes_joints):
        ...
        num_bodies = 1 if ske_joints.shape[1] == 75 else 2
        if num_bodies == 1:
            aligned_skes_joints[idx, :num_frames] = np.hstack((ske_joints, np.zeros_like(ske_joints))) # first: aligned (103, 150) = ske_joints (103,75) + (103,75)
        else:
