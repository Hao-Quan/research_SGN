...
def get_one_actor_points(body_data, num_frames):
    ...
    For joints, each frame contains 75 X-Y-Z coordinates. 75 = 25 x 3
    For colors, each frame contains 25 x 2 (X, Y) coordinates.
    """
    joints = np.zeros((num_frames, 75), dtype=np.float32) # There are 103 frames in first files, 75 coordinates for each frame
    colors = np.ones((num_frames, 1, 25, 2), dtype=np.float32) * np.nan
    start, end = body_data['interval'][0], body_data['interval'][-1] #body_data['interval'] implies [0, 1, 2, ..., 101, 103] if there are 103 frames in one file
    joints[start:end + 1] = body_data['joints'].reshape(-1, 75) # body_data['joints'].shape = 25 x 103 = 25 x #frames in one file
...
def get_raw_denoised_data():
    ...
    for (idx, bodies_data) in enumerate(raw_skes_data):
        ...
        raw_denoised_joints.append(joints) # list contains (#frames x 75)
        raw_denoised_colors.append(colors) # list contains (#frames x 1 x 25 x 2)
        frames_cnt.append(num_frames)   #list contains #frames of each
    ...
    with open(raw_skes_joints_pkl, 'wb') as f:
        pickle.dump(raw_denoised_joints, f, pickle.HIGHEST_PROTOCOL) # store list contains (#frames x 75)

    raw_skes_colors_pkl = osp.join(save_path, 'raw_denoised_colors.pkl')
    with open(raw_skes_colors_pkl, 'wb') as f:
        pickle.dump(raw_denoised_colors, f, pickle.HIGHEST_PROTOCOL)    # store list contains (#frames x 1 x 25 x 2)

    frames_cnt = np.array(frames_cnt, dtype=np.int) # store list contains # frame
